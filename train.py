...
def train(trainloader, validationloader):
    ...
    for epoch in range(dict1['epoch'] + 1, 3):
        ...
        # Create checkpoint after every 5 epochs
        if (not os.path.isdir('./checkpoints')):
            os.mkdir('./checkpoints')
        if ((epoch % 3) == 2):
            dict1 = {
                'Detail': "End to end Super SloMo - without keypoints.",
                'epoch': epoch,
            ...
            torch.save(dict1, "./checkpoints/without_keypoints" + str(checkpoint_counter) + ".ckpt")
